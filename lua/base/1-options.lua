...
vim.opt.undodir = vim.fn.expand("$HOME/.local/share/nvim/undo") --- Set undodir where to store the undo files
vim.opt.updatetime = 300 -- Length of time to wait before triggering the plugin.
vim.opt.virtualedit = "block" -- Allow going past end of line in visual block mode.
vim.opt.writebackup = false -- Disable making a backup before overwriting a file.
vim.opt.shada = "!,'1000,<50,s10,h" -- Remember the last 1000 opened files
vim.opt.history = 10000 -- Number of commands to remember in a history table (per buffer).
...
vim.g.maplocalleader = "à" -- Set default local leader key.
